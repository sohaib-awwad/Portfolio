import{s as y,m as _}from"./projects-BZuKw93H.js";import{s as w}from"./services-n9dh-1Iu.js";import{c as S}from"./projects-sRVvbuZy.js";function k(){const e=document.createElement("section");e.className="container hero",e.innerHTML=`
  ...
  `;const a=e.querySelector(".typing");if(a){const o=["Sohaib Awwad","Floppy Man"];let c=0,s=0,i=!1;const t=()=>{const r=o[c],u=r.substring(0,s);a.textContent=u,!i&&s<r.length?(s++,setTimeout(t,120)):i&&s>0?(s--,setTimeout(t,80)):i?(i=!1,c=(c+1)%o.length,setTimeout(t,400)):(i=!0,setTimeout(t,1500))};t()}return e}function $(){const e=document.createElement("section");return e.className="container stack",e.innerHTML=`
    <h2
      data-aos="fade-up"
  ...
  `,e}function A(){const e=document.createElement("section");return e.className="container about",e.id="about",e.setAttribute("aria-labelledby","about-title"),e.innerHTML=`
  ...
  `,e}const C=(e,a)=>{const c=a%2===0?"fade-right":"fade-left",s=100+a*80,i=y(e.slug),t=(e.cardChips??[]).map(r=>`<li class="chip">${r}</li>`).join("");return`
    ...
    </li>`};function U(){const e=document.createElement("section");return e.className="container services",e.id="services",e.setAttribute("aria-labelledby","services-title"),e.innerHTML=`
  ...
  </ul>`:p("Education details will land here once the new CV is in."),D=e=>e.length?`<div class="resume-skills">
    ${e.map(a=>`
      <div class="resume-skills__group">
        <h3 class="resume-skills__group-title">${a.group}</h3>
        <ul class="resume-skills__chips">
          ${a.items.map(o=>`<li class="chip">${o}</li>`).join("")}
        </ul>
      </div>`).join("")}
  </div>`:p("Professional skills coming soon."),F=e=>e.length?`<ul class="resume-cards">
    ${e.map(a=>`
      <li class="resume-card">
        ${a.logo?`<div class="resume-card__logo"><img src="${a.logo}" alt="" /></div>`:""}
        <div class="resume-card__body">
          ...
          ${a.bullets?.length?`<ul class="resume-card__bullets">${a.bullets.map(o=>`<li>${o}</li>`).join("")}</ul>`:""}
        </div>
      </li>`).join("")}
  </ul>`:p("Work experience details will land here once the new CV is in."),H=e=>e.length?`<ul class="resume-cards">
    ${e.map(a=>`
      <li class="resume-card">
        <div class="resume-card__body">
  ...
  </ul>`:p("Certifications will appear here once the new CV is in."),q=e=>e.length?`<ul class="resume-soft-grid">
    ${e.map(a=>`
  ...
  </ul>`:p("Soft skills coming soon."),V=e=>e.length?`<ul class="resume-languages">
    ${e.map(a=>`
  ...
  </ul>`:p("Languages will land here once the new CV is in."),G=e=>e.length?`<ul class="resume-cards">
    ${e.map(a=>`
      <li class="resume-card">
        <div class="resume-card__body">
  ...
  </ul>`:p("Awards and recognition will land here once the new CV is in."),N=e=>{switch(e){case"education":return j(T);case"pro-skills":return D(I);case"experience":return F(x);case"certifications":return H(E);case"soft-skills":return q(L);case"languages":return V(M);case"awards":return G(P)}},R=()=>{const e=location.hash;if(!e.startsWith(g))return"education";const a=e.slice(g.length);return f.some(o=>o.key===a)?a:"education"};function X(){const e=document.createElement("section");e.className="container resume",e.id="skills",e.setAttribute("aria-labelledby","resume-title");const a=R(),o=f.map(s=>`
    <button
      role="tab"
  ...
  `,B(e),e}function B(e){const a=Array.from(e.querySelectorAll(".resume-tab")),o=Array.from(e.querySelectorAll(".resume-panel"));if(!a.length)return;const c=(s,i={})=>{const{focus:t=!1,updateHash:r=!0}=i;if(a.forEach(n=>{const d=n.dataset.tab===s;n.classList.toggle("is-active",d),n.setAttribute("aria-selected",String(d)),n.setAttribute("tabindex",d?"0":"-1"),t&&d&&n.focus()}),o.forEach(n=>{const d=n.id===`resume-panel-${s}`;n.classList.toggle("is-active",d),d?n.removeAttribute("hidden"):n.setAttribute("hidden","")}),r){const n=`${g}${s}`;location.hash!==n&&history.replaceState(null,"",n)}a.find(n=>n.dataset.tab===s)?.scrollIntoView({behavior:"smooth",inline:"center",block:"nearest"})};a.forEach(s=>{s.addEventListener("click",()=>{const i=s.dataset.tab;c(i)}),s.addEventListener("keydown",i=>{const t=a.indexOf(s);if(t===-1)return;let r=null;switch(i.key){case"ArrowRight":r=(t+1)%a.length;break;case"ArrowLeft":r=(t-1+a.length)%a.length;break;case"Home":r=0;break;case"End":r=a.length-1;break}if(r!==null){i.preventDefault();const u=a[r].dataset.tab;c(u,{focus:!0})}})}),window.addEventListener("hashchange",()=>{if(!location.hash.startsWith(g))return;const s=location.hash.slice(g.length);f.some(i=>i.key===s)&&c(s,{updateHash:!1})})}function W(){const e=document.createElement("section");return e.className="container contact",e.id="contact",e.setAttribute("aria-labelledby","contact-title"),e.innerHTML=`
  ...
  `,e}function J(){const e=document.getElementById("contactForm");if(!e)return;const a=e.querySelector(".form-result"),o=(s,i)=>{const t=e.querySelector(`.error[data-for="${s}"]`);t&&(t.textContent=i)},c=()=>e.querySelectorAll(".error").forEach(s=>s.textContent="");e.addEventListener("submit",s=>{if(s.preventDefault(),c(),a&&(a.textContent=""),e.querySelector(".hp-field").value.trim())return;const t=e.querySelector("#name").value.trim(),r=e.querySelector("#email").value.trim(),u=e.querySelector("#subject").value.trim(),n=e.querySelector("#message").value.trim();let d=!0;if(t||(o("name","Please enter your name."),d=!1),(!r||!/^\S+@\S+\.\S+$/.test(r))&&(o("email","Please enter a valid email."),d=!1),u||(o("subject","Please add a subject."),d=!1),(!n||n.length<10)&&(o("message","Message should be at least 10 characters."),d=!1),!d)return;const h="sohaibawwad8@gmail.com",b=`From: ${t} <${r}>
Subject: ${u}

${n}`,v=`mailto:${encodeURIComponent(h)}?subject=${encodeURIComponent(u)}&body=${encodeURIComponent(b)}`;window.location.href=v,a&&(a.textContent="Opening your email app…",a.classList.add("ok")),e.reset()})}_();const m=document.querySelector("#app");m&&(m.appendChild(k()),m.appendChild($()),m.appendChild(A()),m.appendChild(U()),m.appendChild(X()),m.appendChild(S()),m.appendChild(W()),J(),location.hash&&requestAnimationFrame(()=>{const e=document.querySelector(location.hash);e&&e.scrollIntoView({behavior:"auto",block:"start"})}));
